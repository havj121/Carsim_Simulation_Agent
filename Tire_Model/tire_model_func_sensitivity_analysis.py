...
# 2. ExpTanh 模型定义
def exp_tanh_model(alpha, a1, a2, a3):
    """
    a1: 峰值因子, a2: 饱和区衰减控制, a3: 初始斜率控制
    """
    return a1 * np.tanh(a2 * alpha) * np.exp(-a3 * np.abs(alpha))
...
def plot_tanh_exp_comparison():
    """
    在一个图中分别画出 tanh 函数与 exp 函数 (负指数) 在正半轴的图像
    """
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(x, y_tanh, label='tanh(x)', color='blue', lw=2)
    plt.plot(x, y_exp, label='exp(-x)', color='red', lw=2)
    
    plt.title('Comparison of tanh(x) and exp(-x) on Positive Half-Axis', fontsize=14)
    plt.xlabel('x', fontsize=12)
    plt.ylabel('Function Value', fontsize=12)
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend()
    
    save_path = os.path.join(OUTPUT_DIR, "tanh_vs_exp_positive_axis.png")
    plt.savefig(save_path, bbox_inches='tight')
    plt.close()
    print(f"✅ Saved tanh vs exp comparison plot to {save_path}")
...
if __name__ == "__main__":
    ...
    # 1. 比较 B (Stiffness) 与 a3 (Initial Slope)
    compare_sensitivity('B', 'a3')
    
    # 2. 比较 E (Curvature) 与 a2 (Decay)
    compare_sensitivity('E', 'a2')
    
    print("\n🚀 Plotting tanh vs exp comparison...")
    plot_tanh_exp_comparison()
