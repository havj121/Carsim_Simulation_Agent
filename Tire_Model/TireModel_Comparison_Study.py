...
def run_experiment(exp_name, data_full, model_type, train_indices=None, test_indices=None, axle='front'):
    ...
    print(f"✅ Experiment finished: {exp_name} - {model_type}")
...
def plot_comparisons(all_results):
    ...
    for axle in ['front', 'rear']:
        ...
        # ② 数量影响分析与外延性分析对比图 (2 & 3 vs Baseline)
        for study_name in ["DataVolume", "Extrapolation"]:
            ...
            for m_type in STUDY_CONFIG["models"]:
                ...
                if axle == 'front':
                    err_b = np.abs(np.array(res_b["train_pred"]) - np.array(res_b["train_true"]))
                    err_s = np.abs(np.array(res_s["train_pred"]) - np.array(res_s["train_true"]))
                else:
                    err_b = np.abs(np.sqrt(np.array(res_b["train_pred_fy"])**2 + np.array(res_b["train_pred_fx"])**2) - 
                                   np.sqrt(np.array(res_b["train_true_fy"])**2 + np.array(res_b["train_true_fx"])**2))
